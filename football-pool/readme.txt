...
* For charts, download the [Highcharts Charting Core package](http://www.highcharts.com/download) (see the installation instructions or the help page in the admin).
...
8. To use the chart feature, download the [Highcharts Charting Core package](http://www.highcharts.com/download) and place the `highcharts.js` file from the core folder in your install on the following path `/wp-content/plugins/highcharts-js/highcharts.js`.
...
== Changelog ==
= 2.13.5 =
* Security: Reflected Cross-Site Scripting vulnerability for authenticated users in shoutbox widget.

= 2.13.4 =
* Bug fix: Removed unexisting asset load in admin.

= 2.13.3 =
* Update: Added final 6 teams in CSV for the 2026 FIFA World Cup.

= 2.13.2 =
* New: Added a matches CSV for the 2026 FIFA World Cup (please note that there are still some teams not known).
* New: Option to disable adding new shoutbox messages via the widget (only add via admin area).
* Update: Auto-set user answers option in bonus questions now ignores leading and trailing spaces in answers.
* Update: Removed some deprecated warnings from the CSV import.
* Tweak: Show user ID next to display name in the audit log.
* Tweak: Some refactoring in the $pool->user_name() calls.
* Tweak: Refactoring of global $pool instance. With backwards compatibility for extensions (at least, I hope so).
* Tweak: Reordered the methods in the Football_Pool_Pool class (but it is still a mess).
* Tweak: Small visual change to the cancel/close button of the calculation modal in the admin.
* Tweak: Removed some redundant styling from the default stylesheet and made some small changes.
* Bug fix: Fixed a bug in the format input for 2 shortcodes.
