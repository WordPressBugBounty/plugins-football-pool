...
class Football_Pool_Groups {
	...
	public static function compare_teams( $a, $b ): int {
		// if points are equal
			...
			if ( $a['plays'] === $b['plays'] ) {
				...
				if ( ( $a['for'] - $a['against'] ) === ( $b['for'] - $b['against'] ) ) {
					...
					if ( $a['for'] === $b['for'] ) {
						...
						return ( $teams->get_group_order( $a['team'] ) > $teams->get_group_order( $b['team'] ) ? + 1
							: - 1 );
					}
	...
	public function get_plays( $group_id ): array {
		...
		foreach ( $matches as $match ) {
			if ( in_array( $match['id'], $match_ids, false ) ) {
				$plays[] = $match;
			}
		}
	...
	public function print_group_standing( $group_id, $layout = 'wide', $class = '' ): string {
		if ( $class !== '' ) {
			$class = ' ' . $class;
		}
		...
		$group_row_template = apply_filters( 'footballpool_group_table_group_row_template', $group_row_template,
			$layout );
		...
		foreach ( $ranking as $group => $rank ) {
			if ( $group_id == '' || $group_id == $group ) {
				...
				$template_params = apply_filters( 'footballpool_group_table_template_params', $template_params,
					$group );
				...
				foreach ( $rank as $team_ranking ) {
					...
					if ( $teams->show_team_links ) {
						$row_template_params['team_name'] = sprintf( '<a href="%s">%s</a>'
							, esc_url(
								add_query_arg(
	...
	public function get_ranking_array() {
		...
		$rows = $this->get_group_composition();
		foreach ( $rows as $row ) {
			$group_ranking[ (int) $row['id'] ][ (int) $row['team_id'] ] = $this->get_standing_for_team( $ranking,
				$row['team_id'] );
		}
		...
		foreach ( $rows as $row ) {
			if ( ( $row['home_score'] !== null ) && ( $row['away_score'] !== null ) ) {
				...
				if ( (int) $row['home_score'] > (int) $row['away_score'] ) {
					$wins = $this->set_standing_array( $wins, (int) $row['home_team_id'] );
					$losses = $this->set_standing_array( $losses, (int) $row['away_team_id'] );
				} elseif ( (int) $row['home_score'] < (int) $row['away_score'] ) {
					$losses = $this->set_standing_array( $losses, (int) $row['home_team_id'] );
					$wins = $this->set_standing_array( $wins, (int) $row['away_team_id'] );
				} elseif ( (int) $row['home_score'] === (int) $row['away_score'] ) {
					$draws = $this->set_standing_array( $draws, (int) $row['home_team_id'] );
					$draws = $this->set_standing_array( $draws, (int) $row['away_team_id'] );
				} else {
	...
	private function set_goals_array(
		array &$for,
		array &$against,
		int $home_team,
		int $away_team,
	 ...
	 * @return int
	 */
	private function set_goals( array $goals, int $team, int $score ): int {
		if ( ! isset( $goals[ $team ] ) ) {
			return $score;
		} else {
	 ...
	 * @return array
	 */
	private function set_standing_array( array $arr, int $id ): array {
		if ( isset( $arr[ $id ] ) ) {
			$arr[ $id ] ++;
		} else {
			$arr[ $id ] = 1;
	...
	private function get_standing_for_team( $ranking, $id ): array {
		$team_points_win = Football_Pool_Utils::get_fp_option(
			'team_points_win', FOOTBALLPOOL_TEAM_POINTS_WIN, 'int'
		);
		$team_points_draw = Football_Pool_Utils::get_fp_option(
			'team_points_draw', FOOTBALLPOOL_TEAM_POINTS_DRAW, 'int'
		);

		$wins = $ranking[0][ $id ] ?? 0;
		$draws = $ranking[1][ $id ] ?? 0;
		$losses = $ranking[2][ $id ] ?? 0;
		$for = $ranking[3][ $id ] ?? 0;
		$against = $ranking[4][ $id ] ?? 0;
