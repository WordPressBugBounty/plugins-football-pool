		...
		if ( is_array( $questions) ) {
			foreach( $questions as $question ) {
				if ( $question['match_id'] > 0 ) {
					$match = sprintf(
						'<a href="?page=footballpool-games&amp;item_id=%d&amp;action=edit">%d &gt;&gt;</a>',
						$question['match_id'],
						$question['match_id']
					);
				} else {
				...
				$question_text = strip_tags( $question['question_raw_data'] );
...
//					$question['answer'],
	...
	private static function update_bonus_question( $input ) {
		...
		// Check if the question date is valid, if not default to now.
		...
		if ( $id == 0 ) {
			...
			$sql = $wpdb->prepare(
				"UPDATE {$prefix}bonusquestions_type SET type = %d, options = %s, image = %s, max_answers = %d WHERE question_id = %d",
