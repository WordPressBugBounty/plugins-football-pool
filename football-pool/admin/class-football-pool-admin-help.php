	...
	public static function admin() {
		...
		<div class="help-page">
			...
			There are several ways you can customize the plugin, e.g. different scores for correct answers, add bonus questions,
				...
				<a target="_blank" href="https://wordpress.org/support/plugin/football-pool">WordPress support forum</a>.
			</p>
			...
			<p>
				...
				There is an extra setting for the goal difference bonus that defines the rules for when this bonus will be
				given. You can choose between only rewarding it for toto scores, with or without draws, or for all scores.
			</p>
			<?php
			$rule = Football_Pool_Utils::get_fp_option( 'diffpoints_rule', 0, 'int' );
			$rule_texts = [
				0 => 'Only for toto scores and no draws (classic rule)',
				1 => 'For toto scores and including draws',
				2 => 'For full scores and toto scores, and including draws',
			];
			?>
			<p>
			...
			<table>
				...
				<tr><td>Rules for the goal difference bonus:</td><td><?php echo $rule_texts[$rule]; ?></td></tr>
			</table>
			<p></p>
			...
			<tr>
				...
				<td>
					<?php
						$goaldiff = ( $rule === 2 ) ? ' plus the goal difference bonus' : '';
					?>
					full points plus two times the goal bonus for the correct amount of goals for the home team and
					the away team<?php echo $goaldiff; ?>.<br>
					total = <?php echo $fullpoints; ?> + <?php echo $goalpoints; ?> + <?php echo $goalpoints; ?>
						<?php if ( $rule === 2 ) echo ' + ', $diffpoints; ?> =
						<?php
						self::echo_score(
								2, 1, 2, 1, $joker_multiplier,
								( $fullpoints + ( 2 * $goalpoints ) + ( $rule === 2 ? $diffpoints : 0 ) )
						);
						?>
				</td>
			</tr>
			...
			<tr>
				...
				<td>
					<?php
					$goaldiff = ( $rule !== 0 ) ? ' plus the goal difference bonus' : '';
					?>
					toto points<?php echo $goaldiff; ?>.<br>
					total = <?php if ( $rule !== 0 ) echo $totopoints, ' + ', $diffpoints, ' = '; ?>
					<?php
					self::echo_score( 1, 1, 0, 0, $joker_multiplier, ( $totopoints + ( $rule !== 0 ? $diffpoints : 0 ) ) );
					?>
				</td>
			</tr>
			</table>
...
EOT;
			...
			<p>
				...
				<a href="https://wordpress.org/support/plugin/football-pool" target="_blank">support forum</a> of the
...
EOT;
			...
			<p>
				The CSV file (must be in <a target="_blank"
				                            href="https://superuser.com/questions/479756/eol-in-notepad-and-notepad"
			...
			*/
			...
			<table class="widefat help" caption="Full data">
				...
				<tr>
					...
					<td>https://www.uefa.com/uefaeuro/season=2012/teams/team=95/index.html</td>
				</tr>
				<tr>
				...
				<tr>
					...
					<td>https://www.uefa.com/uefaeuro/season=2012/teams/team=39/index.html</td>
				</tr>
				<tr>
			...
			Just follow the CSS rules about specificity to overwrite the plugin's style (see Keegan Street's <a target="_blank" href="https://specificity.keegan.st/">specificity calculator</a> for a cool help in determining the specificity of a selector). I don't recommend changing the CSS of the plugin, as it will be overwritten on every update.
			</p>
			<h3>Templates</h3>
			...
			<table class="widefat help">
				...
				<tr>
					...
					<td>optional format for the output (uses <a href="https://php.net/sprintf" target="_blank">sprintf</a> notation)</td>
					<td>string</td>
					<td></td>
				</tr>
			</table>
			...
			<p>If you need to hide the original icon in the title, set the constant <span class="code">FOOTBALLPOOL_CHANGE_STATS_TITLE</span> to false in your wp-config file.</p>
			<?php
			...
			<p>
			The charts feature uses the Highcharts API to display the interactive charts. Because of the <a target="_blank" href="https://wordpress.org/extend/plugins/about/">WordPress license guidelines</a> I may not include this library in the package.
			...
			<p>If you want to alter the output or behavior of the plugin there are several hooks you can use. If you want to learn more about hooks, see <a target="_blank" href="https://wp.tutsplus.com/tutorials/plugins/writing-extensible-plugins-with-actions-and-filters/">this tutorial</a> or <a target="_blank" href="https://codex.wordpress.org/Plugin_API">the Codex</a>. Place your custom code in your theme's functions.php file or in your own plugin (<a href="https://codex.wordpress.org/Writing_a_Plugin" target="_blank">how to write your own plugin</a>).</p>
			...
			<div class="help important">
				<p>Please note that some of the examples below use <a title="more on closures" href="https://www.php.net/manual/en/functions.anonymous.php" target="_blank">closures</a>. If you don't have PHP version 5.3 or higher, you'll have to rewrite the example to a named function.</p>
			</div>
...
add_filter( 'footballpool_ranking_ranking_row_template', 
	function( $template, $all_user_view, $type ) {
		if ( $all_user_view ) {
...
// (download it from https://simplehtmldom.sourceforge.net/ and add it to the dir where you placed this script)
...
EOT;
			...
			<p>
				If you have WP-CLI installed (see <a href="https://wp-cli.org" target="_blank">wp-cli.org</a>
...
$
			...
			<p>
	            ...
	            consider a small donation via my <a href="<?php echo FOOTBALLPOOL_DONATE_PAYPALME; ?>" target="_blank">PayPal.me link</a>
				or <a href="<?php echo FOOTBALLPOOL_DONATE_LINK; ?>" target="_blank">donate button</a>.<br>
			...
			<p>
				...
				forum: <a target="_blank" href="https://wordpress.org/support/plugin/football-pool">Support forum</a><br>
