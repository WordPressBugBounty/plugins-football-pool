...
class Football_Pool_Admin_Score_Calculation extends Football_Pool_Admin {
		...
		switch ( $calculation_step ) {
			...
			case 'match_scores':
				...
				if ( $pool->has_matches && count( $match_ids ) > 0 ) {
					...
					while ( $i++ < FOOTBALLPOOL_CALC_STEPSIZE_MATCH ) {
						...
						if ( $simple_calculation_method || $match >= count( $match_ids ) ) {
							...
							// When we are in simple calculation mode, we always move on to the next user
							$user++;
							$match = 0;
				...
				if ( $calculation_step === 'question_scores' ) {
					if ( $pool->has_bonus_questions && count( $question_ids ) > 0 ) {
						$msg = sprintf(
							__( "Updating question scores (step %s of %s)", 'football-pool' ),
							1,
							$sub_iterations[1]
						);
					} else {
			...
			case 'question_scores':
				...
				if ( $calculation_step === 'total_scores' ) {
					$msg = sprintf( __( "Updating total scores (step %s of %s)", 'football-pool' ), 1, $sub_iterations[2] );
				} elseif ( $calculation_step === 'compute_ranking' ) {
					$msg = sprintf( __( 'Calculating the user rankings (step %s of %s).', 'football-pool' ), 1, $sub_iterations[3] );
				}
			...
			case 'total_scores':
				...
				$msg = sprintf(
					__( 'Updating total scores (step %s of %s)', 'football-pool' ),
			...
			case 'finalize':
				...
				update_option(
					FOOTBALLPOOL_LAST_CALC_DATE,
					Football_Pool_Utils::date_from_gmt( date( 'Y-m-d H:i', time() ) )
				);
	 ...
	 * @return string
	 */
	private static function ok_button(): string
	{
	 ...
	 */
	private static function get_score_row(
		int $user_id,
		int $ranking_id,
		string $new_history_table,
		bool $simple_calculation_method ): ?array
	{
	...
	/**
	 * Returns the ranking of users for a ranking id as an array of user ids. The array is sorted top down with higher
