...
class Football_Pool_Statistics_Page {
	...
	private function settings_panel( $panel_content ): string
	{
		if ( $panel_content === '' ) $panel_content = __( 'No settings available', 'football-pool' );
		
		$output = sprintf(
			'<div id="fp-charts-settings">%s<p><input type="submit" value="%s"></p></div>',
			$panel_content,
			__( 'Change charts', 'football-pool' )
		);
		$output .= sprintf(
			'<input type="hidden" name="view" value="%s">', Football_Pool_Utils::get_string( 'view', 'stats' )
		);
		$output .= sprintf(
			'<input type="hidden" name="user" value="%d">', Football_Pool_Utils::get_int( 'user' )
		);
	...
	public function page_content(): string
	{
		...
		if ( $ranking_display == 1 ) {
			...
			$ranking = Football_Pool_Utils::get_fp_option( 'show_ranking', FOOTBALLPOOL_RANKING_DEFAULT, 'int' );
		} else {
			...
			switch ( $view ) {
				...
				case 'user':
					...
					if ( $stats->stats_visible ) {
						...
						$output .= sprintf(
							"<p><a href='%s'>{$txt}</a></p>",
							$url, $pool->user_name( $user_info->ID )
						);
