...
class Football_Pool_Pool {
	 ...
	 * @throws Exception
	 */
	public function __construct( int $season = 0 ) {
		$this->simple_calculation_method =
			Football_Pool_Utils::get_fp_option( 'simple_calculation_method', false, 'bool' );
		...
		$this->has_leagues = Football_Pool_Utils::get_fp_option( 'use_leagues', false, 'bool' )
		                     && ( count( $this->leagues ) > 1 );

		$this->lock_datestring = Football_Pool_Utils::get_fp_option( 'bonus_question_locktime', '' );
		$this->force_lock_time = Football_Pool_Utils::get_fp_option( 'stop_time_method_questions', false, 'bool' )
		...
		$this->always_show_predictions = Football_Pool_Utils::get_fp_option( 'always_show_predictions', false, 'bool' );
	...
	/**
	 * @param  string  $score_table
	 *
	 * @return string
	 */
	private function format_score_table( string $score_table ): string {
	...
	/**
	 * Calculates the score for a given match outcome with the given predictions. Returns an array with the
	 * actual score, plus some extra indicators in the form:
	 *
	 * [
	 *    'score'           => the points for this match;
	 *    'full'            => indicates if it was a full score (1=yes, 0=no);
	 *    'toto'            => indicates if it was a toto score (1=yes, 0=no);
	 *    'goal_bonus'      => indicates how many times the goal bonus was awarded (0, 1 or 2);
	 *    'goal_diff_bonus' => indicates if the goal difference bonus was awarded (1=yes, 0=no);
	 * ]
	 *
	 * @param $home
	 * @param $away
	...
	public function calc_score( $home, $away, $user_home, $user_away, $joker, $match_id, $user_id = null ): array {
		...
		$full_score = $toto_score = $goal_bonus = $goal_diff_bonus = 0;
		...
		$joker_multiplier = Football_Pool_Utils::get_fp_option( 'joker_multiplier', FOOTBALLPOOL_JOKERMULTIPLIER,
			'int' );
		...
		if ( $do_calc ) {
			// No special behavior (like score = '' or score = 0) so we can now calc the score and we start with 0 points
			$score = 0;
			...
			// Check for goal bonus for the home and away predictions
			if ( $home === $user_home ) {
				$score += $goal;
				$goal_bonus++;
			}
			...
			// Check for goal diff bonus (with new rule setup)
			$goal_difference_correct = ( $home - $user_home ) === ( $away - $user_away );
			// Default to the classic rule (to not accidentally ruin existing rankings)
			$goal_diff_bonus_rule = Football_Pool_Utils::get_fp_option( 'diffpoints_rule', 0, 'int' );
			switch ( $goal_diff_bonus_rule ) {
				case 1:
					// For toto scores and including draws
					$award_goal_diff_bonus = ! $full_score && $goal_difference_correct;
					break;
				case 2:
					// For full scores and toto scores, and including draws
					$award_goal_diff_bonus = $goal_difference_correct;
					break;
				default:
					// Only for toto scores and no draws (classic rule)
					$award_goal_diff_bonus = ! $full_score && $match_draw === false && $goal_difference_correct;
			}

			if ( $award_goal_diff_bonus ) {
				$goal_diff_bonus = 1;
				$score += $diff;
			}
	 ...
	 * @throws Exception
	 */
	public function get_bonus_questions_for_user(
		int $user_id = 0,
		?array $question_ids = [],
		bool $only_non_linked = false
	): array {
		if ( $user_id === 0 ) {
			return [];
		}
	...
	public function get_user_score(
		...
		if ( $this->simple_calculation_method ) {
			$score_date = '';
		}
	...
	public function get_amount_of_jokers_allowed(): int {
		if ( Football_Pool_Utils::get_fp_option( 'jokers_enabled', false, 'bool' ) ) {
	 ...
	 * @return array
	 */
	public function get_used_joker_count_per_user(
		array $users,
		int $ranking_id = FOOTBALLPOOL_RANKING_DEFAULT
	): array {
		global $wpdb;
		...
		if ( count( $users ) > 0 ) {
			...
			$user_str = "AND user_id IN ( {$user_str} )";
		} else {
			$user_str = '';
		}

		$sql = $wpdb->prepare(
			"SELECT user_id, SUM( joker_used ) AS num_jokers 
				FROM {$prefix}{$this->scorehistory}
				WHERE ranking_id = %d {$user_str} GROUP BY user_id",
			$ranking_id
		);
		$rows = $wpdb->get_results( $sql, ARRAY_A );
			...
			if ( $user_id <= 0 || ! $pool->user_is_player( $user_id ) ) {
				...
				if ( ! isset( $user_match_info[ $joker ] ) || ! $user_match_info[ $joker ]['match_is_editable'] ) {
					...
					if ( $result === false ) {
						...
						$params['return_code'] = false;
						$params['msg'] = __( 'Something went wrong while saving the multiplier.',
							'football-pool' );
						Football_Pool_Utils::log_message(
							$user_id, FOOTBALLPOOL_TYPE_MATCH, $joker, 0,
							"Match {$joker}: {$home}-{$away} multiplier update failed. " . $params['msg']
	 ...
	 * @return bool
	 */
	private function joker_allowed( int $joker, array $user_match_info, int $user_id ): bool {
		...
		if ( $match_is_editable && Football_Pool_Utils::get_fp_option( 'jokers_enabled', 0, 'int' ) !== 0 ) {
			...
			if ( $this->jokers_per === 1 ) {
				...
				$allowed = $match_type_counted === false
				           || $joker_count[ $match_type_for_joker ] < $number_of_jokers;
			}
		}
	 ...
	 * @return bool
	 */
	public function update_league_for_user(
		int $user_id,
		int $new_league_id,
...
//			$sql = "SELECT
//						q.id, q.question, q.answer, q.points,
//						q.answer_before_date AS question_date,
//						DATE_FORMAT( q.score_date, '%Y-%m-%d %H:%i' ) AS score_date,
//						DATE_FORMAT( q.answer_before_date, '%Y-%m-%d %H:%i' ) AS answer_before_date,
//						q.match_id,
...
//					ORDER BY {$sorting}";
			$sql = "SELECT 
						q.id, q.question, q.answer, q.points, 
						q.answer_before_date, q.score_date, 
						q.match_id,
						qt.type, qt.options, qt.image, qt.max_answers, q.question_order
					FROM {$prefix}bonusquestions q 
					INNER JOIN {$prefix}bonusquestions_type qt
			...
			foreach ( $rows as $row ) {
				$i = (int) $row['id'];
				$answer_before_date = new DateTime( $row['answer_before_date'], new DateTimeZone( 'UTC' ) );

				$question_info[ $i ] = [];
				$question_info[ $i ]['id'] = $i;
				$question_info[ $i ]['question_raw_data'] = $row['question'];
				$question_info[ $i ]['question'] = Football_Pool_Utils::xssafe( $row['question'] );
				$question_info[ $i ]['answer'] = Football_Pool_Utils::xssafe( $row['answer'] );
				$question_info[ $i ]['points'] = $row['points'];
				$question_info[ $i ]['question_timestamp'] = $answer_before_date->format( 'U' );
				...
				$question_info[ $i ]['options'] = Football_Pool_Utils::xssafe( $row['options'] );
	...
	public function get_bonus_question_answers_for_users( int $question_id = 0 ): array {
		...
		// Protect the user answer for XSS attacks
		if ( is_array( $rows ) && count( $rows ) > 0 ) {
			array_walk( $rows, function ( &$v, $k ) {
				$v['answer'] = Football_Pool_Utils::xssafe( $v['answer'], FOOTBALLPOOL_ENCODING, false );
			} );
		}
	...
	public function print_bonus_question( array $question, $nr, bool $is_user_page = false ) {
		$question_is_editable = $this->question_is_editable( $question['question_timestamp'] );
		$show_admin_answer_user_page
			= Football_Pool_Utils::get_fp_option( 'user_page_show_correct_question_answer', false, 'bool' )
			  && $is_user_page;
		$show_admin_answer_prediction_page
			= Football_Pool_Utils::get_fp_option( 'prediction_page_show_correct_question_answer', false, 'bool' )
			  && !$question_is_editable && !$is_user_page;
		...
		if ( $question['image'] !== '' ) {
			$question_img = sprintf(
				'<p class="bonus image"><img src="%s" alt="%s"></p>',
		...
		if ( $question_is_editable ) {
			$title = __( 'answer this question before this date', 'football-pool' );
			$text = __( 'answer before', 'football-pool' );
		} else {
			$title = __( "it's is no longer possible to answer this question, or change your answer", 'football-pool' );
			$text = __( 'closed on', 'football-pool' );
		}
		$closing_time = sprintf( '<div class="closing-time" title="%s">%s %s</div>', $title, $text, $lock_time );

		// Should we show the answer that the admin has set as correct?
		// If not, we return an empty string for this parameter.
		if ( $show_admin_answer_user_page || $show_admin_answer_prediction_page ) {
			$admin_answer = sprintf(
				'<p class="answer correct-answer" id="bonus-%d-correct">%s: ',
				$question['id'],
	...
	private function bonus_question_multiple( array $question, string $type = 'radio' ): string {
		...
		if ( $type === 'select' || $type === 'dropdown' ) {
			...
			foreach ( $options as $option ) {
				...
				if ( $type === 'checkbox' ) {
					$checked = in_array( $option, explode( ';', $question['user_answer'] ?? '' ) )
						? 'checked="checked" ' : '';
					$brackets = '[]';
					$user_input = '';
				} else {
	...
	private function update_bonus_user_answers( array $questions, array $answers, int $user ) {
		...
		foreach ( $questions as $question ) {
			...
			if ( $this->question_is_editable( $question['question_timestamp'] ) && $answer !== '' ) {
				$do_update = apply_filters( 'footballpool_prediction_update_question', $do_update, $user, $question_id,
					$answer );
				if ( $do_update ) {
	 ...
	 */
	public function prediction_form_questions(
		array $questions,
		bool $wrap = false,
		int $id = 1,
		int $start_at_nr = 1,
		...
		if ( $this->has_bonus_questions ) {
			if ( $wrap ) {
				$output .= $this->prediction_form_start( $id );
			}
			...
			if ( $wrap ) {
				$output .= $this->prediction_form_end( $id );
			}
		}
	 ...
	 * @throws Exception
	 */
	public function prediction_form_matches(
		array $matches,
		bool $wrap = false,
		int $id = 1,
		string $type = 'matches'
		...
		if ( $this->has_matches ) {
			...
			$output .= $this->matches->print_matches_for_input( $matches, $id, $user_id, false, $show_actual );
	 ...
	 * @return array
	 */
	public function get_prediction_count_per_user(
		array $users,
		int $ranking_id = FOOTBALLPOOL_RANKING_DEFAULT
	): array {
		global $wpdb;
			...
			if ( $user_id !== 0 && $user_is_player
				...
				if ( $success ) {
					...
					$msg = sprintf(
						...
						,
						__( 'Something went wrong during the save. Check if you are still logged in. If the problems persist, then contact your webmaster.',
							'football-pool' )
					);
				}
			}
	...
	private function update_predictions( int $user_id ): bool {
		...
		if ( $user_id <= 0 || ! $this->user_is_player( $user_id ) ) {
			return false;
		}
						...
						if ( $user_match_info[ $match_id ]['has_prediction'] ) {
							...
							if ( $user_match_info[ $match_id ]['home_score'] != $home
								...
								if ( $result !== false ) {
									...
									$user_match_info[ $match_id ]['away_score'] = $away;
									$match
										= $this->matches->all_matches[ $match_id ];
									$match
									...
									Football_Pool_Utils::log_error_message( $user_id, FOOTBALLPOOL_TYPE_MATCH,
										$match_id );
								}
							}
						} else {
	 ...
	 * @return array
	 */
	public function get_pool_ranking_limited(
		int $league,
		int $num_users,
		int $ranking_id = FOOTBALLPOOL_RANKING_DEFAULT,
		string $score_date = ''
	 ...
	 * @return string
	 */
	public function print_pool_ranking(
		int $league_id,
		int $user_id,
		array $users,
		array $ranking,
		...
		if ( $all_user_view === true ) {
			$ranking_template = '<tr class="%css_class% jokers-used-%jokers_used% multipliers-used-%multipliers_used%">
		...
		if ( $num_users === null || ( is_int( $num_users ) && count( $ranking ) < $num_users ) ) {
			...
			if ( $num_users !== null ) {
				$ranking = array_slice( $ranking, 0, $num_users );
			}
		}
		...
		foreach ( $ranking as $row ) {
			...
			$ranking_template_params = array(
				...
				'user_avatar'        => $this->get_avatar( (int) $row['user_id'], 'medium' ),
	...
	public function get_ranking_by_id( int $id ): ?array {
		...
		$sql = $wpdb->prepare( "SELECT `name`, `user_defined`, `calculate` FROM `{$prefix}rankings` WHERE `id` = %d",
			$id );
