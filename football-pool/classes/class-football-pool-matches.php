...
class Football_Pool_Matches {
	...
	private array $joker_blocked = [ false ];
	private bool $matches_are_editable;
	private bool $force_lock_time;
	private int $lock;
	private bool $use_spin_controls;
	...
	public function __construct() {
		...
		$this->force_lock_time = Football_Pool_Utils::get_fp_option( 'stop_time_method_matches', false, 'bool' )
		                         && ( $datetime !== '' );
		...
		$this->use_spin_controls = Football_Pool_Utils::get_fp_option( 'use_spin_controls', true, 'bool' );
	...
	private function all_matches_info() {
		...
		if ( $match_info === false ) {
			...
			foreach ( $rows as $row ) {
				...
				$match_info[ $i ]['home_score'] = is_numeric( $row['home_score'] ) ? (int) $row['home_score']
					: $row['home_score'];
				$match_info[ $i ]['away_score'] = is_numeric( $row['away_score'] ) ? (int) $row['away_score']
					: $row['away_score'];
				...
				$match_info[ $i ]['match_has_finished'] = is_numeric( $row['home_score'] )
				                                          && is_numeric( $row['away_score'] )
				...
				$match_info[ $i ]['group_id'] = (int) ( $teams->team_info[ (int) $row['home_team_id'] ]['group_id'] ??
				                                        0 );
	...
	public function sort_matches_array_by_date_asc( $a, $b ): int {
		return (int) $a['match_timestamp'] - (int) $b['match_timestamp'];
	}

	public function sort_matches_array_by_date_desc( $a, $b ): int {
	...
	public function get_next_match( $ts = null, $team_id = null ) {
		...
		foreach ( $matches as $match ) {
			if ( $match['match_timestamp'] > $ts
				...
				if ( $prev_ts !== null && $prev_ts != $match['match_timestamp'] ) {
					break;
				}
...
//	}
	...
	public function print_matches( $matches, $page = '' ): string {
		...
		if ( $this->match_table_layout === 0 ) {
			$date_row_template
				= '<tr><td class="matchdate" colspan="6" title="%match_day%">%match_datetime_formatted%</td></tr>';
		} else {
			$date_row_template = '';
		}
		...
		foreach ( $matches as $row ) {
			...
			// CSS class if a match is scheduled for today
			if ( $matchdate->format( 'Y-m-d' ) === current_time( 'Y-m-d', true ) ) {
				$css_class .= ' today';
			}
			...
			if ( $teams->show_team_links ) {
				$match_template_params['home_team'] = sprintf(
				...
				$match_template_params['away_team'] = sprintf(
					'<a title="%s" href="%s">%s</a>',
	 ...
	 * @throws Exception
	 */
	public function format_match_time( $datetime, $format = false ): string {
		if ( $format === false ) {
	...
	private function show_users_link( $match, $ts ): string {
		$output = '';
	 ...
	 * @throws Exception
	 */
	public function print_matches_for_input(
		$matches,
		$form_id,
		$user_id,
		$is_user_page = false,
		...
		if ( $this->match_table_layout === 0 ) {
			$date_row_template
				= '<tr><td class="matchdate" colspan="11" title="%match_day%">%match_datetime_formatted%</td></tr>';
		} else {
			$date_row_template = '';
		}
...
//		$active_matches = $this->get_next_match();
		...
		foreach ( $matches as $row ) {
			...
			// CSS class for non-predicted matches
			if ( ! is_numeric( $row['home_score'] ) && ! is_numeric( $row['away_score'] ) ) {
				$css_class .= ' no-prediction';
			}
			...
			$match_template_params = array(
				...
				'joker'                    =>
					$this->show_pool_joker( $pool, $joker, (int) $info['id'], $info['match_timestamp'], $form_id ),
				'user_score'               => $user_score,
				'user_score_txt'           =>
					is_numeric( $user_score ) ?
						$user_score . ' ' . _n( 'point', 'points', $number, 'football-pool' ) :
						$user_score,
				'css_class'                => $css_class,
				'is_user_page'             => $is_user_page,
				'home_actual_score'        =>
					( $show_actual && is_numeric( $info['home_score'] ) ) ?
						sprintf( $actual_home_score, $info['home_score'] ) : '',
				'away_actual_score'        =>
					( $show_actual && is_numeric( $info['away_score'] ) ) ?
			...
			$match_template_params = apply_filters( 'footballpool_predictionform_match_template_params'
			...
			if ( is_array( $info['linked_questions'] ) && count( $info['linked_questions'] ) > 0 ) {
				...
				foreach ( $questions as $question ) {
					...
					$linked_question_template_params = apply_filters( 'footballpool_linked_question_template_params'
...
//					. '<span class="fp-focus-border"><i></i></span>'
	...
	private function show_pool_joker( $pool, $jokers, $match, $ts, $form_id = 1 ) {
		...
		/*
		Make sure joker is not shown for matches that are editable in case the matches_are_editable property is
		set to false. Unless we have the new 'always display predictions' set, in that case we can ignore this.
		*/
	...
	private function is_joker_blocked( $match_type_id ): bool {
		if ( Football_Pool_Utils::get_fp_option( 'jokers_per', 1, 'int' ) === 1 ) {
			$el = 0;
		} else {
			$el = $match_type_id;
	...
	public function get_joker_value_for_user( $user_id ): array {
		$jokers = [];
	...
	public function get_matches_for_match_type( $ids = array() ): array {
		if ( count( $ids ) === 0 ) {
			return array();
		}
