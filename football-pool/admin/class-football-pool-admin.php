...
class Football_Pool_Admin {
	public static function add_body_class( $classes ) {
		...
		if ( strpos( $hook_suffix, 'footballpool-predictions-audit-log' ) !== false ) $classes .= ' fp-audit-log';
		return $classes;
	}
	...
	public static function init_admin() {
		if ( ! wp_doing_ajax() && ! wp_doing_cron() ) {
			...
			if ( $chart->stats_enabled && ! $chart->API_loaded ) {
				$notice =
					sprintf(
						'<strong>%s: </strong>', __( 'Football Pool', 'football-pool' )
					) .
					__( 'Charts are enabled but Highcharts API was not found!', 'football-pool' ) . ' ' .
					sprintf(
						__( 'See <a href="%s">Help page</a> for details.', 'football-pool' ),
...
//									 'shortcodes' => implode( '|', array_map( array( 'self', 'get_shortcode_codes' ), $shortcodes ) ),
	...
	*/
	...
	public static function link_button( $text, $action, $wrap = false, $other_attributes = null, $type = 'secondary' ): string
	{
