...
class Football_Pool_Shortcodes {
	...
	public static function shortcode_last_predictions( $atts ) {
		...
		$output = '';
		$user_set = [];
		$league = self::league_helper( $atts['league'], 0 );

		if ( $atts['users'] === '' && $atts['top'] === '' ) {
			$user_set[] = get_current_user_id();
		} elseif ( $atts['top'] !== '' && is_numeric( $atts['top'] ) && (int) $atts['top'] > 0 ) {
			$ranking_users = $pool->get_pool_ranking_limited( $league, (int)$atts['num'], (int)$atts['ranking'], 'now' );
	...
	public static function shortcode_user_list( $atts ): string
	{
		...
		$league = self::league_helper( $atts['league'], $user_id );
		$num = is_numeric( $atts['num'] ) ? (int) $atts['num'] : 0; // Default to 0 if not a number
		$latest = ( $atts['latest'] === 'yes' );
		...
		if ( is_numeric( $league ) && $league >= 0 ) {
			...
			$template_end = '</ol>';
			$template_end = apply_filters( 'footballpool_fp-user-list_template_end',
				$template_end, $league, $user_id
			);
			...
			$row_template = apply_filters( 'footballpool_fp-user-list_template_row',
				$row_template, $league, $user_id
			);
			
			$template_params = [];
			$template_params = apply_filters( 'footballpool_fp-user-list_template_params',
				$template_params, $league, $user_id
			);

			$users = $pool->get_users( $league, 'user_registered DESC, user_name ASC' );

			// Should we limit the results to a max number of users?
			if ( $num > 0 && count( $users ) > 0 ) {
				if ( $latest === true ) {
				...
				if ( $info === 'name' ) {
					...
					if ( $info === 'numplayers' ) {
						...
						if ( $info === 'points' ) {
							...
							foreach ( $rows as $row ) {
								$w = $num_predictions[$row['user_id']] ?? 0;
								$x = $row['points'];
								$wx = $w * $x;
								$sum_w += $w;
								$sum_wx += $wx;
	...
	public static function shortcode_group( $atts ) {
		...
		if ( is_numeric( $atts['id'] ) && array_key_exists( (int) $atts['id'], $group_names ) ) {
			$output = $groups->print_group_standing( (int) $atts['id'], 'wide', 'shortcode' );
		}
	...
	public static function shortcode_ranking( $atts ) {
		...
		if ( ! is_numeric( $atts['num'] ) || $atts['num'] <= 0 ) {
			$num = $default_num;
		} else {
			$num = (int) $atts['num'];
		}
		...
		if ( ! is_numeric( $atts['ranking'] ) || $atts['ranking'] <= 0 ) {
			$ranking_id = FOOTBALLPOOL_RANKING_DEFAULT;
		} else {
			$ranking_id = (int) $atts['ranking'];
		}
		
		$rows = $pool->get_pool_ranking_limited( $league, $num, $ranking_id, self::date_helper( $atts['date'] ) );
