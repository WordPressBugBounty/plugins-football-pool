...
class Football_Pool_Chart_Data {
	...
	*************************************************/
	...
	public function points_total_pie_chart_data( $user, $ranking_id = FOOTBALLPOOL_RANKING_DEFAULT ) {
		...
		$output['total_score'] = ( $data !== null ) ? (int) $data : 0;
		...
		// Determine the max points a player can get for a match (excluding the multiplier).
		...
		$full = $full + ( 2 * $goal );
		// For a toto score you get the toto score and possibly (at max) also one goal bonus.
		$toto = $toto + $goal;

		$goal_diff_bonus_rule = Football_Pool_Utils::get_fp_option( 'diffpoints_rule', 0, 'int' );
		// For all cases of the diff rule
		...
		if ( $goal_diff_bonus_rule === 2 ) {
			$full += $diff;
		}

		$max_points_per_match = max( $full, $toto );

		$output['max_score'] = 0;

		// Account for the available multipliers
		$num_jokers = $pool->get_amount_of_jokers_allowed();
		if ( $num_jokers > 0 ) {
			$jokers_per = Football_Pool_Utils::get_fp_option( 'jokers_per', 1, 'int' );

			// Loop through matches per match type (possibly per match type)
			$jokers_counted = 0;
			foreach( $match_types as $match_ids ) {
				if ( $jokers_per === 2 ) $jokers_counted = 0;
				foreach( $match_ids as $match_id ) {
					if ( $jokers_counted++ < $num_jokers ) {
						$output['max_score'] += $max_points_per_match * $joker_multiplier;
					} else {
						$output['max_score'] += $max_points_per_match;
					}
				}
			}
		} else {
			$output['max_score'] += $num_matches * $max_points_per_match;
		}
	...
	******************************************/
	public function score_chart_series( $rows ) {
		...
		return apply_filters( 'footballpool_score_chart_series', $data, $rows );
	}
	...
	public function predictions_pie_series( $row ) {
		...
		return apply_filters( 'footballpool_predictions_pie_series', $data, $row );
	}
	...
	public function points_total_pie_series( $row ) {
		...
		return apply_filters( 'footballpool_points_total_pie_series', $data, $row );
	}
	...
	public function bonus_question_pie_series( $rows, $open = 'open' ) {
		...
		return apply_filters( 'footballpool_bonus_question_pie_series', $data, $rows, $open );
	}
	...
	public function bonus_question_pie_series_one_question( $row ) {
		...
		return apply_filters( 'footballpool_bonus_question_pie_series_one_question', $data, $row );
	}
