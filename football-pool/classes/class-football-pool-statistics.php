...
class Football_Pool_Statistics {
	...
	public bool $stats_visible = false;
	public bool $stats_enabled = false;
	private bool $always_show_predictions = false;
	...
	public function data_available_for_match( $match ): bool
	{
	...
	public function show_match_info( $info ): string
	{
		...
		if ( count( $info ) > 0 ) {
			if ( $this->always_show_predictions || ! $info['match_is_editable'] ) {
		...
		return apply_filters( 'footballpool_statistics_show_match_info', $output, $info );
	}
	...
	public function show_bonus_question_info( $question ): string
	{
		...
		if ( $info ) {
			...
			$output .= sprintf( '<h2 class="q%d">%s</h2>', $info['id'], $info['question'] );
			...
			if ( $this->always_show_predictions || $info['question_is_editable'] == false ) {
				...
				if ( ! $info['question_is_editable'] ) {
					$output .= sprintf(
						'<br><span class="question-answer">%s: %s</span>',
	...
	public function show_answers_for_bonus_question( $id ) {
		...
		foreach ( $rows as $answer ) {
			...
			$row_params['user_answer'] = $answer['answer'];
	...
	public function show_predictions_for_match( $match_info ) {
		...
		if ( count( $rows ) > 0 ) {
			...
			$template_start = sprintf(
				'<table class="matchinfo match-%d %s statistics">
					<thead>
					<tr>
						<th class="username">%s</th>
						<th colspan="%d">%s</th>
						<th>%s</th>
					</tr>
					</thead><tbody>',
			...
			$row_template = '<tr class="%current_user_css_class% %score_indicators%">
			...
			foreach ( $rows as $row ) {
				$multiplier_set = ( (int)$row['has_joker'] === 1 );
				...
				if ( $multiplier_set ) {
					$row_params['joker_title_text'] = _x(
						'multiplier set',
						'to indicate that a user has set a multiplier for this match',
						'football-pool'
					);
				...
				$row_params['score_indicators'] = '';
				if ( $score['full'] === 1 ) $row_params['score_indicators'] .= 'full-score ';
				if ( $score['toto'] === 1 ) $row_params['score_indicators'] .= 'toto-score ';
				if ( $score['goal_bonus'] === 1 ) $row_params['score_indicators'] .= 'goal-bonus-1x ';
				if ( $score['goal_bonus'] === 2 ) $row_params['score_indicators'] .= 'goal-bonus-2x ';
				if ( $score['goal_diff_bonus'] === 1 ) $row_params['score_indicators'] .= 'goal-diff-bonus ';
				if ( $multiplier_set ) $row_params['score_indicators'] .= 'multiplier-set ';
