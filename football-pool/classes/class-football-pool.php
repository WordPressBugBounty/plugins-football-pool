...
class Football_Pool {
	...
	public static function activate( ?string $action = 'install' ): void
	{
		...
		$options['prediction_page_show_correct_question_answer'] = 0; // defaults to false
		...
		if ( $action === 'install' ) {
			...
			if ( ! self::is_at_least_version( '2.12.0' ) ) {
				$update_sql = self::prepare( self::read_from_file( FOOTBALLPOOL_PLUGIN_DIR . 'data/update-2.12.0.txt' ) );
				self::db_actions( $update_sql );
				// Set the 'goaldiff rule' to classic for existing installs
				Football_Pool_Utils::update_fp_option( 'diffpoints_rule', 0 );
			}
	...
	public static function show_admin_bar( $content ) {
		$show_bar = true;
		if ( Football_Pool_Utils::get_fp_option( 'hide_admin_bar', 1, 'int' ) === 1 ) {
			$show_bar = false;
			// only admins get the admin bar after login when the hide admin bar option is set
			if (
				current_user_can( 'manage_options' ) ||
				current_user_can( FOOTBALLPOOL_ADMIN_BASE_CAPABILITY ) ||
				current_user_can( FOOTBALLPOOL_ADMIN_MATCHES_CAPABILITY ) ||
				current_user_can( FOOTBALLPOOL_ADMIN_QUESTIONS_CAPABILITY )
			) {
				$show_bar = true;
			}
		}
