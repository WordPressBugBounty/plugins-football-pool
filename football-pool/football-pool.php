...
//require_once 'classes/class-football-pool-match.php';
...
// Widgets (always load these, except for cron; so, front-end, admin and ajax).
if ( ! wp_doing_cron() ) {
...
add_filter( 'wp_privacy_personal_data_exporters',
	[ 'Football_Pool_Admin_Personal_Data', 'register_user_data_exporters' ] );
if ( Football_Pool_Utils::get_fp_option( 'erase_personal_data', 0, 'int' ) === 1 ) {
	add_filter( 'wp_privacy_personal_data_erasers',
		[ 'Football_Pool_Admin_Personal_Data', 'register_privacy_erasers' ] );
}

// Needed for admin, cron and wp-cli.
